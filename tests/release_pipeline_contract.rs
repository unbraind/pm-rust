...
fn changelog_toolchain_pins_the_sdk_exactly() -> Result<(), BoxError> {
    let justfile = read_repo_file("justfile")?;
    assert!(
        justfile.contains("PM_CHANGELOG_PKG := \"pm-changelog@2026.9.25\""),
        "the justfile must pin pm-changelog exactly"
    );
    assert!(
        justfile.contains("PM_CLI_PKG := \"@unbrained/pm-cli@2026.9.26\""),
            ...
            for line in logical_lines(run) {
                ...
                assert!(
                    line.contains("--package=pm-changelog@2026.9.25")
                        && line.contains("--package=@unbrained/pm-cli@2026.9.26"),
