...
fn changelog_toolchain_pins_the_sdk_exactly() -> Result<(), BoxError> {
    let justfile = read_repo_file("justfile")?;
    assert!(
        justfile.contains("PM_CHANGELOG_PKG := \"pm-changelog@2026.9.16\""),
        "the justfile must pin pm-changelog exactly"
    );
    assert!(
        justfile.contains("PM_CLI_PKG := \"@unbrained/pm-cli@2026.9.17\""),
    ...
    for (job_name, steps) in workflow_jobs(&workflow)? {
        for step in &steps {
            ...
            for line in logical_lines(run) {
                ...
                assert!(
                    line.contains("--package=pm-changelog@2026.9.16")
                        && line.contains("--package=@unbrained/pm-cli@2026.9.17"),
